<template>
  <div class="container">
    ...
    <form v-if="canCreate" @submit.prevent="createApartment" class="apartment-form">
    ...
    <div v-if="filteredApartments.length">
      <ul class="apartment-list">
        ...
        >
          ...
          <button v-if="canDelete" @click="deleteApartment(apartment.id)" class="delete-button">
            Delete
          </button>
          <button v-if="canEdit" @click="openEditModal(apartment)" class="edit-button">
            Edit
          </button>
        </li>
      </ul>
...
export default {
  ...
  data() {
    return {
      ...
      isModalOpen: false,
      userRole: 3, // Example: 1 = Admin, 2 = Moderator, 3 = User
    };
  },
  computed: {
    filteredApartments() {
  ...
  created() {
    this.loadApartments();
    this.getUserRole(); // Assuming you have a method to fetch/set user role
  },
  methods: {
    async loadApartments() {
      try {
        ...
        await axios.get("/dailyRents").then(response => {
    ...
    async createApartment() {
      try {
        ...
        await axios.post("/dailyRent", this.newApartment);
    ...
    async deleteApartment(id) {
      ...
      try {
        ...
        await axios.delete(`/dailyRent/${id}`);
        await this.loadApartments();
      } catch (error) {
    ...
    async updateApartment() {
      ...
      try {
        ...
        await axios.put(`/dailyRent`, this.editingApartment);
        await this.loadApartments();
        this.closeModal();
      } catch (error) {
    ...
    searchApartments() {
      // Optional: Implement search logic if needed
    },
  },
};
</script>
